...
public class Node {
    ...
    public Node(State state) {
        this(state, null, null, 0); // why DEPTH is not initialized
    }
    ...
    public static Node manhattanDistance(State initial, List fringe) {                                                  //manhattan Distance
        int status=0, min=1000;
        // add the initial state to the fringe
        fringe.add(new Node(initial));
        Node tree = new Node(initial);
        
        System.out.println("Bobot Manhattan Distance = "+initial.countManhattanDistance());
        // loop until find goal state
        while (status <= 20) {
            if (initial.goal()) {
                return tree; // if goal state then return this node
            }
            // Expand all possible move from current state
            List possibleMove = Arrays.asList(tree.expand());
            // Loop trough all possible move to find lowest distance
